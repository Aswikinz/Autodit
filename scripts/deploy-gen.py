...
target=root/'deploy/compose/compose.json';target.parent.mkdir(parents=True,exist_ok=True);target.write_text(json.dumps(compose,indent=2)+'\n')
...
'''
target=root/'deploy/systemd/autodit.service';target.parent.mkdir(parents=True,exist_ok=True);target.write_text(unit)
